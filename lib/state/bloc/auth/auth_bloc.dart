  ...
  AuthBloc() : super(AuthInitial()) {
    ...
    on<AuthRequestToken>((event, emit) async {
      ...
      if (responseToken.statusCode == 200 &&
          responseToken.data['success'] == true) {
    ...
    on<AuthExchangeToken>((event, emit) async {
      ...
      try {
        ...
        var responseSesion = await dio.post(sessionURl,
            data: {'request_token': event.token},
    ...
    on<AuthDenied>((event, emit) {
      emit(AuthInitial());
      emit(AuthFailed(
          e: 'Access denied. Please click the "Approve" button to authorize the app. This is Youre Last Chance'));
      // emit(AuthInitial());
    });
    on<AuthlogOut>((event, emit) async {
      ...
      try {
        var response = await dio.delete(outUrl,
            data: {"session_id": sesionId},
            options: Options(headers: {
              'Content-Type': 'application/json',
            }));
